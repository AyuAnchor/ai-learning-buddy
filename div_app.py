...
def generate_quiz(topic: str, num_questions: int) -> List[Dict[str, Any]]:
    ...
    prompt = f"""
    Generate a quiz with exactly {num_questions} questions on the topic "{topic}" for a student who is preparing for the Joint Entrance Exam (JEE).  
    Pick the questions from existing previous‐year questions (PYQs) available for the JEE when possible.
    ...
    3. Return **only** valid JSON (an array of question objects), with **no** additional text.

    Format the response as a JSON array of objects; each object must look exactly like this example (note the doubled braces `{{` and `}}` around the object):
...
def display_quiz():
    ...
    with left_column:
        ...
        def submit_cb(idx):
            ...
            sel = st.session_state[f"answer_{idx}"]
            correct_idx = q['correctAnswer']
            if sel == q['answers'][correct_idx]:
                st.session_state.last_verdict = ("success", "✅ Correct! (+4 points)")
                ...
                st.session_state.last_verdict = (
                    "error",
                    f"❌ Incorrect (-1 point). The correct answer was: {correct_text}"
                )
                st.session_state.score -= 1

            st.session_state.current_explanation = q['explanation']
            st.session_state.user_answered = True
            st.session_state.total_attempted += 1

        def skip_cb(idx):
            q = st.session_state.quiz_questions[idx]
            correct_text = q['answers'][q['correctAnswer']]
            st.session_state.last_verdict = (
                "info",
                "ℹ️ Question skipped. "
            ...
            st.session_state.current_explanation = (
                f"Skipped. The correct answer was: {correct_text}. "
                + q['explanation']
            )
            st.session_state.user_answered = True
            st.session_state.total_attempted += 1
        ...
        if current_q < total_questions:
            ...
            # Show Submit/Skip only if not answered
            if not st.session_state.user_answered:
                c1, c2 = st.columns(2)
                with c1:
                    st.button(
                        "Submit Answer",
                        key=f"submit_{current_q}",
                        on_click=partial(submit_cb, current_q)
                    )
                with c2:
                    st.button(
                        "Skip Question",
                        key=f"skip_{current_q}",
                        on_click=partial(skip_cb, current_q)
                    )

            # Once answered, show stored verdict
            if st.session_state.user_answered and st.session_state.last_verdict:
                v_type, v_text = st.session_state.last_verdict
                {
                    "success": st.success,
                    "error":   st.error,
                    "info":    st.info
                ...
                with b1:
                    if not st.session_state.show_explanation:
                        st.button(
                            "View Explanation",
                            key=f"view_exp_{current_q}",
                            on_click=view_cb
                        )
                    else:
                        st.button(
                            "Hide Explanation",
                            key=f"hide_exp_{current_q}",
                            on_click=hide_cb
                        )
                with b2:
                    st.button(
    ...
    with right_column:
        if current_q < total_questions:
            ...
            attempted = current_q + (1 if st.session_state.user_answered else 0)
            acc = (
                st.session_state.correct_answers / attempted * 100
                if attempted else 0
            )
            st.markdown(f"**Score:** {st.session_state.score}")
            st.markdown(f"**Question:** {current_q + 1}/{total_questions}")
            st.markdown(f"**Accuracy:** {acc:.1f}%")
            st.progress(attempted / total_questions)
            if acc >= 75:
